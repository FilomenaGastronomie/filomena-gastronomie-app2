...
import { CatalogProduct, DeliveryType, EncomendaRevenueRecord } from "@/lib/types";
...
export function OrderNotes({ initialText, updatedAt, initialRecords, catalogProducts }: Props) {
  ...
  const [tipoEntrega, setTipoEntrega] = useState<DeliveryType>("retirada");
  ...
  const monthlyCalculator = useMemo(() => {
    const filteredByMonth = records.filter((record) => getMonthValue(record.data) === monthlyCalculatorMonth);

    return {
      registros: filteredByMonth.length,
      faturamento: filteredByMonth.reduce((acc, record) => acc + record.valor, 0),
      lucro: filteredByMonth.reduce((acc, record) => acc + (record.valor - record.custo), 0),
    };
  }, [monthlyCalculatorMonth, records]);
  ...
  function startEditing(record: EncomendaRevenueRecord) {
    ...
    setTipoEntrega(record.tipoEntrega ?? "retirada");
    ...
    <section ref={formRef} className="card narrow-card">
      ...
      <div className="product-reference-box">
        ...
        <div className="metrics-grid metrics-grid-wide">
          <div className="metric">
            <span>Registros</span>
            <strong>{monthlyCalculator.registros}</strong>
          </div>
          <div className="metric">
      ...
      <div className="form-grid">
        ...
        <label>
          ...
          <select value={tipoEntrega} onChange={(event) => setTipoEntrega(event.target.value as DeliveryType)}>
      ...
      {selectedRecord ? (
        <div className="detail-panel section-space">
          ...
          <div className="detail-grid">
            ...
            <div className="metric">
              <span>Horário</span>
              <strong>{selectedRecord.horario || "-"}</strong>
            </div>
            <div className="metric">
              <span>Entrega</span>
              <strong>{selectedRecord.tipoEntrega === "entrega" ? "Entrega" : "Retirada"}</strong>
            </div>
            <div className="metric">
              <span>Valor</span>
        ...
        {records.map((record) => (
          <div key={record.id} className="record-card">
            ...
            <div>
              ...
              <span>
                {record.tipoEntrega === "entrega" ? "Entrega" : "Retirada"} • {record.horario || "-"}
              </span>
            </div>
            <div>
